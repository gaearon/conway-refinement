...
OVERVIEW = ROOT / "blueprint/src/overview.tex"
...
def render_content(nodes: dict[str, Node]) -> str:
    ...
    for phase in PHASE_METADATA:
        ...
        if not labels:
            continue
        lines.extend([f"\\section{{{phase.title}}}", "", phase.description, ""])
        for label in labels:
            node = nodes[label]
...
def render_overview() -> str:
    return "\n".join(
        [
            ...
            "\\section*{Overview}",
            "",
            HIGHLIGHTS_METADATA.description,
            "",
            "\\clearpage",
        ]
    )
...
def main() -> None:
    ...
    changed = []
    if update(OVERVIEW, overview, args.check):
        changed.append(OVERVIEW.relative_to(ROOT).as_posix())
