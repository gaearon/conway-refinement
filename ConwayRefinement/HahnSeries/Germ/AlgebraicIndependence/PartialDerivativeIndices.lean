...
This file contains the ordinal bookkeeping for partial derivatives at a variable whose part of the
degree below `β` precedes `λ₀` in the algebraic order. It identifies the cofactor degrees and proves
the inequalities needed by the well-founded cofactor construction. The arguments depend only on
the ordinal data of a relation whose degree is a limit ordinal and on the chosen cutoff, not on the
exponent group.
-/
...
open MvPolynomial HahnSeries HahnSeries.Nonpositive DirectSum OrdinalGraded
...
/-- If generators outside a subtype contribute to no degree in `[τ, h)`, they may be removed
from an ideal-membership statement for the components at or above `τ`. -/
theorem componentsGE_mem_span_subtype {ι : Type w} [Finite ι]
    {q : ι → MvPolynomial σ K} {c : ι → NatOrdinal}
    (hq : ∀ j, IsWeightedHomogeneous wt (q j) (c j)) {P : MvPolynomial σ K}
    {τ h : NatOrdinal} (hP : componentsGE wt τ P ∈ Ideal.span (Set.range q))
    (hdeg : ∀ d ∈ P.support, Finsupp.weight wt d < h) (A : ι → Prop)
  ...
  have heh : Finsupp.weight wt d < h := hdeg d hd
  have hmem : weightedHomogeneousComponent wt (Finsupp.weight wt d) P ∈
      Ideal.span (Set.range q) :=
    weightedHomogeneousComponent_mem_span_of_componentsGE_mem wt hq hP hτe
  obtain ⟨a, -, ha0, hsum⟩ := exists_decompose_eq_sum_mul_of_mem_span
    (𝒜 := weightedHomogeneousSubmodule K wt)
    (fun j ↦ (mem_weightedHomogeneousSubmodule _ _ _ _).mpr (hq j)) hmem
    (Finsupp.weight wt d)
  rw [hdec] at hsum
  ...
  have hzero : ∑ j ∈ Finset.univ.filter (fun j ↦ ¬ A j), q j * a j = 0 :=
    Finset.sum_eq_zero fun j hj ↦ by
      rw [ha0 j (hdrop j (Finset.mem_filter.mp hj).2 _ hτe heh), mul_zero]
  rw [hzero, add_zero]
...
omit hx in
/-- The degree of the partial derivative at a variable of `F` is nonzero. -/
...
omit hv in
/-- The degree of a partial derivative has zero finite part. -/
...
omit hx hv in
/-- The partial derivative at `v` is homogeneous of complementary degree `c`. -/
...
theorem partGE_cdeg_eq_self_of_lowDegreePartEq (htop : S.LowDegreePartEq v) :
  ...
  have h := NatOrdinal.partGE_add_partLT S.β c
  rwa [S.partLT_cdeg_eq_zero_of_lowDegreePartEq hc htop, add_zero] at h
...
theorem partLT_τ (h : NatOrdinal) :
  ...
  have h' := NatOrdinal.partGE_add_partLT S.β (NatOrdinal.partGE S.β h + lam')
  rw [S.partGE_τ hlam' h] at h'
...
/-- The cutoff attached to `v'` lies at least two successor steps below its partial degree. -/
...
theorem partGE_eq_of_τ_le {e : NatOrdinal}
  ...
  le_antisymm (NatOrdinal.partGE_mono heh.le) (by
    have h' := NatOrdinal.partGE_mono (β := S.β) hτe
    rwa [S.partGE_τ hlam' h] at h')
...
theorem partGE_le_τ_of_hasProperLowDegreePartNotAlgebraicLE
  ...
  have hεβ : ε ≤ S.β := by
    have h1 := NatOrdinal.leastTerm_le
      (S.partLT_cdeg_ne_zero hc ((S.hasProperLowDegreePartNotAlgebraicLE_iff v).mp hL).2.2.1)
    rw [hε] at h1
  ...
  calc
    NatOrdinal.partGE ε h = NatOrdinal.partGE ε (NatOrdinal.partGE S.β h) +
        NatOrdinal.partGE ε (NatOrdinal.partLT S.β h) := by
...
theorem not_algebraicLE_of_lowDegreePartEq_of_not (htop : S.LowDegreePartEq v)
    (hdrop : ¬ NatOrdinal.AlgebraicLE c h) {e : NatOrdinal}
    (hτe : NatOrdinal.partGE S.β h + lam' ≤ e) (heh : e < h) :
...
theorem not_algebraicLE_of_hasProperLowDegreePartNotAlgebraicLE_of_not
  ...
  have h1 : NatOrdinal.AlgebraicLE (NatOrdinal.partGE S.β c)
      (NatOrdinal.partGE S.β h) := by
    have h' := hce.partGE S.β
    rwa [hGE] at h'
  ...
  have hlamεs : lam' ≤ s := by
    have h2 : NatOrdinal.partGE S.β h + lam' ≤ NatOrdinal.partGE S.β h + s := by
      calc
        NatOrdinal.partGE S.β h + lam' ≤ e := hτe
  ...
  obtain ⟨ε, hε⟩ := NatOrdinal.exists_leastTerm_eq_wpow
    (S.partLT_cdeg_ne_zero hc ((S.hasProperLowDegreePartNotAlgebraicLE_iff v).mp hL).2.2.1)
  have h5 := S.partGE_lam₀_eq_of_hasProperLowDegreePartNotAlgebraicLE
    hx hL (S.partLT_cdeg_add_degLT hc) hε
  have hlamε : NatOrdinal.partGE ε lam' =
  ...
  have h2 : NatOrdinal.AlgebraicLE (NatOrdinal.partLT S.β c)
      (NatOrdinal.partLT S.β h) := by
