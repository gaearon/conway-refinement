...
instance tailSubmodule_isConvex (T : Set (FiniteArchimedeanClass G)) :
    (tailSubmodule K T).toAddSubgroup.IsConvex := by
...
theorem mk_quotientProdLinearEquiv (x : (G ⧸ P) × P) :
    Quotient.mk (quotientProdLinearEquiv P x) = x.1 := by
...
noncomputable def quotientLexEquiv [P.toAddSubgroup.IsConvex] :
    ((G ⧸ P) ×ₗ P) ≃+o G :=
    ...
    map_le_map_iff' := by
      ...
      apply (show StrictMono
        ...
        · have hq :
            ...
            change (Quotient.mk (quotientProdLinearEquiv P (ofLex x)) : G ⧸ P) <
              Quotient.mk (quotientProdLinearEquiv P (ofLex y))
...
@[simp]
theorem quotientLexEquiv_apply [P.toAddSubgroup.IsConvex] (x : (G ⧸ P) ×ₗ P) :
