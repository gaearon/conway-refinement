...
theorem mem_of_nonneg_of_le (H : AddSubgroup G) [H.IsConvex] {x y : G} (hx : 0 ≤ x)
    (hxy : x ≤ y) (hy : y ∈ H) : x ∈ H :=
...
theorem mk_le_mk_iff {a b : G} :
  ...
  · rintro ⟨a', b', ha', hb', hab⟩
    ...
    have hmem := AddSubgroup.mem_of_nonneg_of_le H (sub_nonneg.mpr h.le) hsub
      (H.sub_mem hb' ha')
  ...
  le_trans := by
    ...
    · rcases hbc with hbc | hbc
      ...
      · rcases le_or_gt a c with h | h
        ...
        have hmem := AddSubgroup.mem_of_nonneg_of_le H (sub_nonneg.mpr h.le)
