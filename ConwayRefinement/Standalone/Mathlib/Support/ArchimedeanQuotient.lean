...
public import ConwayRefinement.Standalone.Mathlib.Support.ConvexQuotient
...
instance tailKernel_isConvex (T : Set (FiniteArchimedeanClass G)) :
    ConvexQuotient.IsConvex (tailKernel T) where
  ordConnected := by
    constructor
...
theorem quotient_noMax_of_eq_tailKernel (T : Set (FiniteArchimedeanClass G))
    (H : AddSubgroup G) [ConvexQuotient.IsConvex H] (hH : H = tailKernel T)
    (hT : ∀ c ∈ T, ∃ d ∈ T, c < d) :
