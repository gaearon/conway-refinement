...
private theorem normalForm_support_nonnegative : normalForm.support ⊆ Ici 0 := by
  ...
  | coe n =>
      rw [mem_Ici]
      change (0 : Surreal) ≤ (exponent n : ℝ)
      exact_mod_cast (exponent_pos n).le

/-- Conway's one-row normal form, regarded as an omnific integer. -/
def oneRowOz : Oz.OmnificInteger :=
  ⟨normalForm, by
    rw [Oz.mem_omnificIntegers]
    exact ⟨normalForm_support_nonnegative,
      ⟨1, by simpa using normalForm_coeff_zero.symm⟩⟩⟩

@[simp]
theorem oneRowOz_val : oneRowOz.1 = normalForm := by
  rfl
